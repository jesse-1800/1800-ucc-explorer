    ...
    public static function initialize($alias)
    {
        try {
            if (array_key_exists($alias, self::$cxn)) {
                ...
                // For some reason, the code above doesn't work with Dreamhost.

                $servername = self::$cxn[$alias]['hostname'];
                $database = self::$cxn[$alias]['database'];
                $username = self::$cxn[$alias]['username'];
                $password = self::$cxn[$alias]['password'];
