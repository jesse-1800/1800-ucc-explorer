...
if (!function_exists('page_error')) {
    ...
    {
        // This is edited to let Vue handle the 404/error pages
        #include("." . VIEWS_PATH . "errors/{$n}.php");
        include("." . VIEWS_PATH . SYSTEM_MODE . TEMPLATE_TYPE);
        return exit();
    }
}
...
if (!function_exists('system_signature')) {
    /**
     * Determines whether the request
     * is an AJAX request.
     *
    ...
    function system_signature()
    {
        return <<<EOF
            ...
            Best regards,<br>
            VirtuSync Admin<br>
            <a href="tel:(561) 783-8110">(561) 783-8110</a><br>
            <a href="mailto:info@cadrecrew.com">info@cadrecrew.com</a>
        EOF;
    }
}
...
if (!function_exists('get_client_ip')) {
    ...
    {
        // Order of preference: real client -> X-Forwarded-For -> REMOTE_ADDR
        $ip = $_SERVER['REMOTE_ADDR'] ?? null;

        // Common proxy headers (you can add more)
        $proxyHeaders = [
            'HTTP_X_FORWARDED_FOR',
            'HTTP_CLIENT_IP',
            'HTTP_CF_CONNECTING_IP',   // Cloudflare
            'HTTP_TRUE_CLIENT_IP',     // Akamai/Fastly
            'HTTP_X_REAL_IP',          // Nginx (sometimes used)
        ];

        foreach ($proxyHeaders as $header) {
            if (!empty($_SERVER[$header])) {
                // X-Forwarded-For can contain multiple IPs, the first is the client
                $forwarded = explode(',', $_SERVER[$header]);
...
if (!function_exists('logger')) {
    function logger($content, $file = 'app.log')
    {
